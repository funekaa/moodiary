...
class MergeUtil {
  static Future<void> merge({required String lastAppVersion}) async {
    /// 数据库版本变更
    /// v2.4.8
    if (PackageUtil.compareVersion(lastAppVersion, '2.4.8') < 0) {
      await compute(
    ...
    /// v2.6.0
    if (PackageUtil.compareVersion(lastAppVersion, '2.6.0') < 0) {
      await compute(
    ...
    /// v2.6.2
    /// 修复部分视频缩略图无法生成的问题
    if (PackageUtil.compareVersion(lastAppVersion, '2.6.2') < 0) {
    ...
    /// 修复同步失败导致本地分类丢失
    /// 视频缩略图重复生成
    if (PackageUtil.compareVersion(lastAppVersion, '2.6.3') < 0) {
    ...
    /// v2.7.3
    /// 修改自定义字体存储方式
    if (PackageUtil.compareVersion(lastAppVersion, '2.7.3') < 0) {
